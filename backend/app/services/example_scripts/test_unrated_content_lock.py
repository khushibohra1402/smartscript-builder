"""
Test Case: Unrated Content Lock/Unlock Validation
Description:
Validate the functionality of locking and unlocking unrated content on the STB based on the configured age rating settings.
Steps:
- Navigate to parental control settings
- Configure age rating and enable unrated content lock
- Attempt to access unrated content
- Verify that access is restricted when lock is enabled
- Disable the unrated content lock
- Attempt to access the same content again
- Verify that access is allowed when lock is disabled
Expected:
Unrated content should be restricted or accessible based on the configured lock settings.
Preconditions:
- Device should be powered on and accessible
- Parental control settings should be available
OCR:
Tesseract
"""
